<?php
	class Items_model extends CI_Model{
		...
		//die($id.'=='.$this->session->userdata('role_id'));
			...
			$wh[] ='item_discard_status=0';
			$SQL ='SELECT *, CONCAT(grade_name_en, \'-\', subject_name_en) AS gradesubject 
					FROM 
						ci_items 
					LEFT JOIN 
						ci_grades ON grade_id = item_grade_id 
					LEFT JOIN 
						ci_subjects ON subject_id = item_subject_id 
					LEFT JOIN
						ci_subcontent_stands ON subcs_id = item_subcstand_id 
					LEFT JOIN 
						ci_admin ON item_submittedby = admin_id';				
			if(count($wh)>0)
			{
				$WHERE = implode(' and ',$wh);
				return $this->datatable->LoadJson($SQL,$WHERE);
...
///////////////////////////////////////////////////
		...
		{	
			...
			else
			{
				$this->db->select('grade_id,grade_code,grade_name_en,grade_name_ur')
						 ...
						 ->join('ci_grades', 'grade_id = subject_gradeid', 'left')
						 ->where('subject_id IN ('.$this->session->userdata('subjects_ids').')')
						 ->group_by('subject_gradeid');				
				$query = $this->db->get();
				return $result = $query->result_array();
			}
		...
		{	
			$this->db->select('board_id,board_code,board_name_en,board_name_ur')
					 ->from('ci_boards')					 
					 ->where('board_status', 1)
					 ->order_by('board_name_en','asc');
			$query = $this->db->get();
			return $result = $query->result_array();
		}
		public function get_all_bloomtaxs()
		{	
			$this->db->select('bloomtax_id,bloomtax_code,bloomtax_name_en,bloomtax_name_ur')
					 ->from('ci_bloom_taxonomy')					 
					 ->where('bloomtax_status', 1)
					  ->order_by('bloomtax_id','asc');;
			$query = $this->db->get();
			return $result = $query->result_array();
		}
		public function get_all_difficulty_levels()
		{	
			$this->db->select('difficultylevel_id,difficultylevel_code,difficultylevel_name_en,difficultylevel_name_ur')
					 ->from('ci_difficulty_level')					 
					 ->where('difficultylevel_status', 1)
					  ->order_by('difficultylevel_name_en','asc');;
			$query = $this->db->get();
			return $result = $query->result_array();
		}
		public function get_all_solo_levels()
		{	
			$this->db->select('sololevel_id,sololevel_code,sololevel_name_en,sololevel_name_ur')
					 ->from('ci_solo_level')					 
					 ->where('sololevel_status', 1)
					  ->order_by('sololevel_name_en','asc');;
			$query = $this->db->get();
			return $result = $query->result_array();
		}
///////////////////////////////////////////////////Get Subjects by Grade using ajax call	
		public function get_ae_subjects_by_grade($grade_id,$subjectList,$subject_board_id)
		{
			$this->db->select('*')
					 ->from('ci_subjects')
					 ->join('ci_grades', 'grade_id = subject_gradeid')
					 ->where('subject_gradeid', $grade_id)
					 ->where('subject_board_id', $subject_board_id);
...
///////////////////////////////////////////////////////////////////
		...
		public function get_subjects_by_grade_board_series($grade_id,$subject_board_id,$subject_series_id)
		{
			$this->db->select('*')
					 ->from('ci_subjects')
					 ...
					 ->where('subject_gradeid', $grade_id)
					 ->where('subject_board_id', $subject_board_id)
					 ->where('subject_series_id', $subject_series_id)
					 //->where('subject_id IN ('.$this->session->userdata('subjects_ids').')')
					 ->where('subject_status', 1);
			if($this->session->userdata('role_id') == 3)
			{
		...
		public function get_subjects_by_iw()
		{
			$this->db->select('*')
					 ->from('ci_subjects')
					 ->join('ci_grades', 'grade_id = subject_gradeid')//'item_subject_id IN ('.$subjectList.')'
...
///////////////////////////////////////////////////Get Contenstand by subject using ajax call	
		...
		{
			$this->db->select('COUNT(item_id)+1 AS maxitems, subject_code, (subject_gradeid) AS grade')
					 ->from('ci_subjects')
...
///////////////////////////////////////////////////Get SubContentStand by Content Stand using ajax call
		...
		function get_subcstands_by_cstand_subject($cs_id, $item_subject_id)
		{
			$this->db->select('subcs_id,subcs_number,subcs_topic_en,subcs_topic_ur')
					 ->from('ci_subcontent_stands')
					 ->where('subcs_cstand_id', $cs_id)
					 ->where('subcs_subject_id', $item_subject_id)
					 ->where('subcs_status', 1);					 
			$query = $this->db->get();			
			return $result = $query->result_array();
		}
		
		function get_subcstands_by_cstand_subject_multi($cs_id, $item_subject_id)
		{
			$this->db->select('subcs_id,subcs_number,subcs_topic_en,subcs_topic_ur')
					 ->from('ci_subcontent_stands')
					 ->where_in('subcs_cstand_id', $cs_id)
					 ->where('subcs_subject_id', $item_subject_id)
					 ->where('subcs_status', 1);					 
			$query = $this->db->get();			
			return $result = $query->result_array();
		...
		function get_subcstands_by_subject($subcs_subject_id)
		{
			$this->db->select('subcs_id,subcs_number,subcs_topic_en,subcs_topic_ur')
					 ->from('ci_subcontent_stands')
					 ->where('subcs_subject_id', $subcs_subject_id)
					 ->where('subcs_status', 1);					 
			$query = $this->db->get();			
			return $result = $query->result_array();
		}
		function get_subcstands_by_subject_cstrand($subcs_cstand_id, $subcs_subject_id)
		{
			$this->db->select('subcs_id,subcs_number,subcs_topic_en,subcs_topic_ur')
					 ->from('ci_subcontent_stands')
					 ->where('subcs_subject_id', $subcs_subject_id)
					 ->where('subcs_cstand_id', $subcs_cstand_id)
					 ->where('subcs_status', 1);					 
			$query = $this->db->get();			
			return $result = $query->result_array();
		}
		function get_subcstands_by_subject_cstrand_multi($subcs_cstand_id, $subcs_subject_id)
		{
			$this->db->select('subcs_id,subcs_number,subcs_topic_en,subcs_topic_ur')
					 ->from('ci_subcontent_stands')
					 ->where('subcs_subject_id', $subcs_subject_id)
					 ->where_in('subcs_cstand_id', $subcs_cstand_id)
					 ->where('subcs_status', 1);					 
			$query = $this->db->get();			
			return $result = $query->result_array();
		}
...
//////////////////////////////////////////////////
		...
		public function get_item_by_id($item_id)
		{
			$this->db->select('*')
					 ->from('ci_items')
					 ->join('ci_boards', 'board_id = item_curriculum', 'left')
					 ->join('ci_bloom_taxonomy', 'bloomtax_id  = item_cognitive_bloom', 'left')
					 ->join('ci_grades', 'grade_id = item_grade_id', 'left')
					 ->join('ci_subjects', 'subject_id = item_subject_id', 'left')
					 ->join('ci_content_stands', 'cs_id = item_cstand_id', 'left')
					 ->join('ci_subcontent_stands', 'subcs_id = item_subcstand_id', 'left')
					 ->join('ci_series', 'series_id = subject_series_id', 'left')
					 ->join('ci_admin', 'item_submittedby = admin_id', 'left')
		...
		public function get_all_subcstands_subject($subcs_subject_id)
		{	
			$this->db->select('subcs_id,subcs_topic_en,subcs_topic_ur,subcs_number')
					 ->from('ci_subcontent_stands')					 
					 ->where('subcs_status', 1)
...
SELECT COUNT(para_id) AS founded FROM `ci_items_paragraphs` WHERE para_item_21 = 10274 OR para_item_22 = 10274  OR para_item_23 = 10274 OR para_item_24 = 10274 OR para_item_25 = 2079 OR para_item_26 = 10274 OR para_item_27 = 10274 OR para_item_28 = 10274 OR para_item_29 = 10274 OR para_item_30 = 10274*/
		...
		public function get_item_history($id)
		{
			...
			$this->db->select('*')
					 ...
					 //->join('ci_boards', 'board_id = item_curriculum') 
					 ->join('ci_grades', 'grade_id = item_grade_id')
					 ->join('ci_subjects', 'subject_id = item_subject_id')
					 ->join('ci_content_stands', 'cs_id = item_cstand_id')
					 ->join('ci_subcontent_stands', 'subcs_id = item_subcstand_id')
		...
		*/
		...
		public function get_all_app_items($item_lang, $item_curriculum, $subject_series_id, $item_grade_id, $item_subject_id, $item_cstand_id, $item_subcstand_id, $item_submittedby, $item_type)
		{
			if($item_lang != '0')
				$wh[] ="item_lang = '$item_lang'";				
			if($item_curriculum != 0) 
				$wh[] ='item_curriculum = '.$item_curriculum;
			if($subject_series_id != 0) 
				$wh[] ='subject_series_id = '.$subject_series_id;
			if($item_grade_id != 0) 
				$wh[] ='item_grade_id = '.$item_grade_id;
			if($item_subject_id != 0) 
				$wh[] ='item_subject_id = '.$item_subject_id;
			if($item_cstand_id != 0) 
				$wh[] ='item_cstand_id = '.$item_cstand_id;
			//if($item_subcstand_id != 0) 
				//$wh[] ='item_subcstand_id = '.$item_subcstand_id;
			//if($item_type != '0')
				//$wh[] ="item_type = '$item_type'";
			
			if($item_subcstand_id != 0)
			{
				$item_subcstand_ids = explode('_',$item_subcstand_id);
				$wh[] ='item_subcstand_id IN ('.implode(',',$item_subcstand_ids).')';	
			}
			
			if($item_submittedby != 0)
			{
				$item_submittedby_ids = explode('_',$item_submittedby);
				$wh[] ='admin_id IN ('.implode(',',$item_submittedby_ids).')';	
			}
				
			if($item_type != '0')
			{
				$itemtypearr = array();
				$item_types = explode('_',$item_type);
				foreach($item_types as $item_type)
				{
					$itemtypearr[] = "'$item_type'";
				}
				$wh[] ='item_type IN ('.implode(',',$itemtypearr).')';	
			}
				
			$wh[] ='item_status IN (2,4)';
			$SQL ='SELECT *, CONCAT(grade_name_en, \'-\', subject_name_en) AS gradesubject 
					FROM 
						ci_items 
					LEFT JOIN 
					...
					LEFT JOIN
						ci_subcontent_stands ON subcs_id = item_subcstand_id 	
					LEFT JOIN 
						ci_admin ON admin_id = item_submittedby';		
			if(count($wh)>0)
			{
				$WHERE = implode(' and ',$wh);
				return $this->datatable->LoadJson($SQL,$WHERE);
		...
		public function get_all_app_items_multi($item_lang, $item_curriculum, $subject_series_id, $item_grade_id, $item_subject_id, $item_cstand_id, $item_subcstand_id, $item_submittedby, $item_type)
		{
			if($item_lang != '0')
				$wh[] ="item_lang = '$item_lang'";				
			if($item_curriculum != 0) 
			...
			if($item_cstand_id != 0)
			{
				$item_cstand_id = explode('_',$item_cstand_id);
				$wh[] ='item_cstand_id IN ('.implode(',',$item_cstand_id).')';	
			}
			...
			$wh[] ='item_status IN (2,4)';
			$SQL ='SELECT item_id, item_type, item_option_correct, item_lang, 
			item_option_a_en, item_option_b_en, item_option_c_en, item_option_d_en,
			item_option_a_ur, item_option_b_ur, item_option_c_ur, item_option_d_ur,
			item_stem_en, item_stem_ur, grade_name_en, subcs_topic_en,username,item_fib_key_ur,item_fib_key_eng,
			 item_cognitive_bloom , CONCAT(grade_name_en, \'-\', subject_name_en) AS gradesubject 
					FROM 
						ci_items 
					LEFT JOIN 
						ci_grades ON grade_id = item_grade_id 
		...
		public function get_all_app_items_multi_iwsubjects($item_lang, $item_curriculum, $subject_series_id, $item_grade_id, $item_subject_id, $item_cstand_id, $item_subcstand_id, $item_submittedby, $item_type)
	{
		$wh = [];
	
		if ($item_lang !== '0') $wh[] = "ci_items.item_lang = ".$this->db->escape($item_lang);
		if ($item_curriculum != 0) $wh[] = "ci_items.item_curriculum = ".intval($item_curriculum);
		if ($subject_series_id != 0) $wh[] = "ci_subjects.subject_series_id = ".intval($subject_series_id);
		if ($item_grade_id != 0) $wh[] = "ci_items.item_grade_id = ".intval($item_grade_id);
		if ($item_subject_id != 0) $wh[] = "ci_items.item_subject_id = ".intval($item_subject_id);
		
		if ($item_cstand_id != 0) {
			$ids = implode(',', array_map('intval', explode('_', $item_cstand_id)));
			$wh[] = "ci_items.item_cstand_id IN ($ids)";
		}
	
		if ($item_subcstand_id != 0) {
			$ids = implode(',', array_map('intval', explode('_', $item_subcstand_id)));
			$wh[] = "ci_items.item_subcstand_id IN ($ids)";
		}
	
		if ($item_submittedby != 0) {
			$ids = implode(',', array_map('intval', explode('_', $item_submittedby)));
			//$wh[] = "ci_items.item_submittedby IN ($ids)";
		}
	
		if ($item_type != '0') {
			$itemtypearr = implode(',', array_map([$this->db, 'escape'], explode('_', $item_type)));
			$wh[] = "ci_items.item_type IN ($itemtypearr)";
		}
	
		// Add role-based subject filtering
		if ($this->session->userdata('subjects_ids')) {
			$wh[] = "ci_subjects.subject_id IN (".$this->session->userdata('subjects_ids').")";
		}
	
		$wh[] = "ci_items.item_status IN (2,4)";
	
		$whereStr = implode(" AND ", $wh);
	
		$SQL = "SELECT 
			ci_items.item_id, ci_items.item_type, ci_items.item_option_correct, ci_items.item_lang, 
			ci_items.item_option_a_en, ci_items.item_option_b_en, ci_items.item_option_c_en, ci_items.item_option_d_en,
			ci_items.item_option_a_ur, ci_items.item_option_b_ur, ci_items.item_option_c_ur, ci_items.item_option_d_ur,
			ci_items.item_stem_en, ci_items.item_stem_ur, 
			ci_items.item_fib_key_ur, ci_items.item_fib_key_eng,
			ci_items.item_cognitive_bloom, 
			ci_grades.grade_name_en, 
			ci_subcontent_stands.subcs_topic_en,
			ci_admin.username,
			CONCAT(ci_grades.grade_name_en, '-', ci_subjects.subject_name_en) AS gradesubject
		FROM ci_items
		LEFT JOIN ci_grades ON ci_grades.grade_id = ci_items.item_grade_id
		LEFT JOIN ci_subjects ON ci_subjects.subject_id = ci_items.item_subject_id
		LEFT JOIN ci_subcontent_stands ON ci_subcontent_stands.subcs_id = ci_items.item_subcstand_id
		LEFT JOIN ci_admin ON ci_admin.admin_id = ci_items.item_submittedby";
	
		return (count($wh) > 0)
			? $this->datatable->LoadJson($SQL, $whereStr)
			: $this->datatable->LoadJson($SQL);
	}

		
		public function get_app_items_for_export($item_lang, $item_curriculum, $subject_series_id, $item_grade_id, $item_subject_id, $item_cstand_id, $item_subcstand_id, $item_submittedby, $item_type)
		{
			$this->db->select("(@row_number := @row_number + 1) AS srno,
                   item_id, 
						 CASE item_cognitive_bloom
                       WHEN 1 THEN 'Remembering'
                       WHEN 2 THEN 'Understanding'
                       WHEN 3 THEN 'Applying'
                       WHEN 4 THEN 'Analysing'
                       WHEN 5 THEN 'Evaluating'
                       ELSE 'Creating'
                   END AS item_cognitive_bloom,
                   item_code, 
                   item_stem_en, 
                   item_stem_ur,
                   grade_name_en, 
                   ...
                   subcs_topic_en, 
                   username")
					 ->from('(SELECT @row_number := 0) AS t, ci_items')
					 ->join('ci_grades', 'grade_id= item_grade_id')
					 ->join('ci_subjects', 'subject_id = item_subject_id')
					 ->join('ci_content_stands', 'cs_id = item_cstand_id')
					 ->join('ci_subcontent_stands', 'subcs_id = item_subcstand_id')
					 ->join('ci_admin', 'admin_id= item_submittedby')
					 ->where('item_status IN (2,4)')
					 ->order_by('item_id','desc');
					 
			if($item_lang != '0')
				$this->db->where('item_lang', $item_lang);
			if($item_curriculum != 0) 
				$this->db->where('item_curriculum',$item_curriculum);
			if($subject_series_id != 0) 
				$this->db->where('subject_series_id',$subject_series_id);
			if($item_grade_id != 0) 
			...
			//if($item_type != '0' || !empty($item_type))
				//$this->db->where_in('item_type', $item_type);
			
			if($item_cstand_id != 0)
			{
				$item_cstand_id = explode('_',$item_cstand_id);
				$this->db->where_in('item_cstand_id', $item_cstand_id);
			}
			if($item_subcstand_id != 0)
			{
				$item_subcstand_ids = explode('_',$item_subcstand_id);
				$this->db->where_in('item_subcstand_id', $item_subcstand_ids);
			}
			...
			if($item_submittedby != 0)
			{
				$item_submittedby_ids = explode('_',$item_submittedby);
				if($this->session->userdata('role_id') != 3)
					$this->db->where_in('admin_id', $item_submittedby_ids);
			}
			if($this->session->userdata('role_id') == 3)
				$this->db->where('subject_id IN ('.$this->session->userdata('subjects_ids').')');	
			...
			if($item_type != '0')
			{
				$itemtypearr = array();
				$item_types = explode('_',$item_type);
				$this->db->where_in('item_type', $item_types);
			}			 
			$query = $this->db->get();
		...
		public function get_subjects_by_series($grade_id,$subjectList,$subject_board_id, $i_series_id)
		{
			$this->db->select('*')
					 ->from('ci_subjects')
					 ->join('ci_grades', 'grade_id = subject_gradeid')
					 ->where('subject_gradeid', $grade_id)
					 ->where('subject_board_id', $subject_board_id)
					 ->where('subject_series_id', $i_series_id);
			if($this->session->userdata('role_id') !=1){
                   $this->db->where('subject_id IN ('.$subjectList.')');
                   $this->db->where('subject_status', 1);	
                }
