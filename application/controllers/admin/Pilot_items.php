...
class Pilot_items extends MY_Controller
{
	...
	public function pilot_ss_view23($id = 0)
	{
		$data['title'] = 'View Item Filmzy';
		$data['grades'] = $this->Pilot_Items_model->get_all_grades();
		$data['items'] = $this->Pilot_Items_model->get_pilot_item_by_id23($id);
		$data['iwinfo'] = $this->Pilot_Items_model->get_iwinfo_by_id($data['items'][0]->item_submittedby);
		//$data['items_logs'] = $this->Pilot_Items_model->get_item_logs($id);

		$this->load->view('admin/includes/_header', $data);
		$this->load->view('admin/pilot_items/pilot_ss_items_view23', $data);
		$this->load->view('admin/includes/_footer');
	}
	...
	{
		echo json_encode($this->Pilot_Items_model->get_cstands_by_subject($this->input->post('subject_id')));
	}

	public function subcstands_by_cstand()
	{
		echo json_encode($this->Pilot_Items_model->get_subcstands_by_cstand($this->input->post('cs_id')));
	}

	public function subjects_by_grade()
	{
		$subjectList = $this->session->userdata('subjects_ids');
		echo json_encode($this->Pilot_Items_model->get_ss_subjects_by_grade($this->input->post('grade_id'), $subjectList));
	}
	...
	public function pilot_ss_items_search()
	{
		if ($this->input->post('submit_search')) {
			if ($this->input->post('search_subject') == '' && $this->input->post('search_grade') == '' && $this->input->post('search_cstrand') == '' && $this->input->post('search_subcstrand') == '') {
				redirect(base_url('admin/pilot_items/mcq_p1'));
			} else {
				$subjectList = $this->session->userdata('subjects_ids');
				...
				$this->load->view('admin/includes/_header', $data);
				$this->load->view('admin/pilot_items/mcq_p1_list', $data);
				$this->load->view('admin/includes/_footer');
			}
		}
	}

	public function datatable_json_mcq_p1_search($id = 0)
	{
		$records = [];
		$records = $this->Pilot_Items_model->get_all_items_SS_searched($id);
	...
	public function pilot_view_combine23($id = 0)
	{
		$data['title'] = 'View Item Filmzy';
		$data['grades'] = $this->Pilot_Items_model->get_all_grades();
		$data['items'] = $this->Pilot_Items_model->get_pilot_item_by_id23($id);
		...
		$this->load->view('admin/includes/_header', $data);
		$this->load->view('admin/pilot_items/pilot_items_view_combine23', $data);
		$this->load->view('admin/includes/_footer');
	}
	public function pilot_view_combine24($id = 0)
	{
		$data['title'] = 'View Item Filmzy';
		$data['grades'] = $this->Pilot_Items_model->get_all_grades();
		$data['items'] = $this->Pilot_Items_model->get_pilot_item_by_id24($id);
		$data['history'] = $this->Items_model->get_item_history($id);
		$data['rev_history'] = $this->Items_model->get_rev_item_history($id);
		//$data['logs'] = $this->Items_model->get_item_logs($id);
		$data['items_logs'] = $this->Pilot_Items_model->get_item_logs($id);
		...
		$this->load->view('admin/includes/_header', $data);
		$this->load->view('admin/pilot_items/pilot_items_view_combine24', $data);
		$this->load->view('admin/includes/_footer');
	}
