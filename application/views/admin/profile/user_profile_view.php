...
<div class="content-wrapper">
	...
	<?php if($this->session->userdata('admin_id') != $this->uri->segment(4)){
				redirect(base_url('admin/profile/profile_view/'.$this->session->userdata('admin_id')), 'refresh');
				}
		  ?>
			...
			<div class="card-body">
				...
				<div class="row">
					...
					<div class="col-lg-3 col-sm-12">
						...
						<?php if ($user['image'] != ""){ ?><p class="profilepara"><a href="<?php echo base_url().$user['image'];?>" target="_blank"><img src="<?php echo base_url().$user['image'];?>" style="max-height:100px; max-width:100px;"/> </a></p>
				...
				<div class="form-group" id="div_sel_roles">
					...
					if ( $user[ 'admin_role_id' ] == 2 ) {?>
						<label for="parent_admin_id" class="col-sm-12 ">Assessment Expert</label>
					<?php }else{?>
						<label for="parent_admin_id" class="col-sm-12 ">Subject Specialist</label>
					<?php }?>
					<?php
					if ( $user[ 'admin_role_id' ] == 2 ) {
					...
					foreach ( $aes as $ae ) {
						if ( $ae[ 'admin_id' ] == $user[ 'parent_admin_id' ] ){
							print '<p class="profilepara">'.$ae[ 'username' ].'</p>';
						}
					}
					?>
				...
				<div class="form-group" id="div_sel_subjects">
					<div class="col-lg-12 col-sm-12">
						...
						foreach ( $subjects as $subject ) {
							...
							?>
						<div style="float:left; padding:0 20px; width:290px;"><input disabled type="checkbox" class="subs" name="subjects[]" value="<?= $subject['subject_id'];?>" <?php if(in_array($subject[ 'subject_id'],$arrSubjects)){ echo 'checked'; } ?> />
				...
				<div class="row form-group">
					<div class="col-lg-3 col-sm-12 form-group" id="div_sel_district">
						...
						foreach($districts as $district)
						{
							...
							if($district['district_id'] == $user['u_district_id'])
								print '<p class="profilepara">'.$district[ 'district_name_en' ].'</p>';

						}
						?>
					</div>
					<div class="col-lg-3 col-sm-12" id="div_sel_tehsil">
