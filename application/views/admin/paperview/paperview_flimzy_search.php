...
<div class="content-wrapper">
  <section class="content">
      ...
      <div class="card-header">
        <div class="d-inline-block">
		  ...
		  <?php
			...
			$typeofquestion_id 		= (isset($search_typeofquestion) && $search_typeofquestion != "")?$search_typeofquestion:0;
			?>
		<div class="d-inline-block float-right">
          <div class="btn-group margin-bottom-20"> 
            <a href="<?=base_url('admin/paperview/create_flimzy_pdf?grade_id='.$grade_id.'&subject_id='.$subject_id.'&cstand_id='.$cstand_id.'&subcstand_id='.$subcstand_id.'&typeofquestion_id='.$typeofquestion_id);?>" class="btn btn-secondary" style="margin:05px">Export as PDF</a>
          </div>         
        </div>
      </div>
    </div>
    <div class="card">
      <div class="card-header">
            <?php echo form_open(base_url('admin/paperview/search_paperview_flimzy'), 'class="form-horizontal" method="post" onSubmit="return checkFields();" ');  ?>
              ...
              <div class ="col-12">
              	<div class ="row" style="padding-top:25px">
                    ...
                    <div class ="col-3">
                    	<select name="search_typeofquestion" class="form-control form-group" id="search_typeofquestion"  style="width:100%" required="required">						
                          <option value="">---Select Type of Question---</option>
                          <option value="MCQ" <?php if($search_typeofquestion=='MCQ'){?> selected="selected"<?php }?>>MCQs</option>
                          <option value="ERQ" <?php if($search_typeofquestion=='ERQ'){?> selected="selected"<?php }?>>ERQs</option>
							<option value="Paragraphs" <?php if($search_typeofquestion=='Paragraphs'){?> selected="selected"<?php }?>>Paragraphs</option>
							<option value="Groups" <?php if($search_typeofquestion=='Groups'){?> selected="selected"<?php }?>>Groups</option>
                        </select>
                    </div>
					<div class ="col-3">&nbsp;</div>
					<div class ="col-3">&nbsp;</div>
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url('admin/paperview/datatable_jsons_ae_accepted_flimzy?grade_id='.$grade_id.'&subject_id='.$subject_id.'&cstand_id='.$cstand_id.'&subcstand_id='.$subcstand_id.'&typeofquestion_id='.$typeofquestion_id);?>",
    "order": [[1,'desc']],
    "columnDefs": [
