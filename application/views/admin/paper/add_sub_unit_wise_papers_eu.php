  ...
  <section class="content">
    <div class="card card-default color-palette-bo">
      ...
      <div class="card-body">
        ...
        <?php echo form_open(base_url('admin/paper/add_sub_unit_wise_papers_eu'), 'class="form-horizontal" method="post"');  ?>
        <div class="row">
          ...
          <div class="col-lg-3 col-sm-12">
            <label for="qat_cs_id" class="control-label">Select CS *</label>
            <select name="qat_cs_id" class="form-control form-group" id="qat_cs_id" required>
              <option value="">--Select CS--</option>
            </select>
          </div>
          <div class="col-lg-3 col-sm-12">
            <label for="qat_subcs_id" class="control-label">Select Unit *</label>
            <select name="qat_subcs_id[]" class="form-control form-group" id="qat_subcs_id" required multiple="multiple">
...
$('#qat_cs_id').on('change', function() {
	//$('#paper_section').val('');
    $.post('<?=base_url("admin/paper/subcstands_by_cstand_sub")?>',
    {
      '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
      qat_cs_id : this.value
    },
    function(data){
		arr = $.parseJSON(data);     
		console.log(arr.length);     
		...
		$('#qat_subcs_id').trigger('chosen:updated');   
    });	
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#qat_subcs_id").chosen({disable_search_threshold: 10});
	});	
</script>