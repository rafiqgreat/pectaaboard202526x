				...
				<div id="wizard">
					...
					for($i = 1; $i <= $numberOfBlocks; $i++){?>
						...
						<section>
							...
							<h3><strong>Select Group Block-<?php print $i;?></strong></h3>
						...
						for($j = 1; $j <= $numberOfQuestions; $j++){?>
							...
							<div class="row">
								...
								<div class="col-lg-12 col-sm-12">
									...
									<h3><strong>Group #<?php print $i.'.'.$j;?> </strong></h3>
									...
									<select name="ibc_b<?php print $i.'_group'.$j;?>" class="form-control form-group" id="ibc_b<?php print $i.'_group'.$j;?>" onChange="getCurrentValue(this.id, this.value, <?php print $numberOfBlocks;?>, <?php print $numberOfQuestions;?>)">
										...
										foreach($groups as $group){
											...
											if($selectedItem['ibc_group_id'] == $group->group_id)

												 $selected = ' selected="selected" ';

											print '<option value="'.$group->group_id.'" '.$selected.'>'.$group->group_id.' '.cleanstring($group->group_title_en).' - '.cleanstring($group->group_title_ur).' - Q1. '.cleanstring($group->group_item_1_stem_en).' Q2. '.cleanstring($group->group_item_2_stem_en).'</option>';
			...
			$.each( arr, function ( key, value ) {
				...
				$( '#ibc_b' + i + '_group' + j )
					...
					.append( $( "<option></option>" )
						...
						.text( value.group_id + ' ' + clean_en_string+' - '+clean_ur_string+' - Q1. '+clean_group_item_1_stem_en+' - Q2. '+clean_group_item_2_stem_en )
