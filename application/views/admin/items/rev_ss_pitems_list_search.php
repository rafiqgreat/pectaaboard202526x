    ...
    <div class="card">
      <div class="card-header">
        <div class="d-inline-block">
          <h3 class="card-title"><i class="fa fa-list"></i>&nbsp; SS Pending Items List (Searched)</h3>
        </div>
            ...
            <?php echo form_open(base_url('admin/items/rev_ss_pitems_search'), 'class="form-horizontal" method="post" onSubmit="return checkFields();" ');  ?>
              ...
              <div class ="col-10">
              	<div class ="row">
                    ...
                    <div class ="col-4">
                    	<select name="item_reviewedby" class="form-control form-group" id="item_reviewedby"  style="width:100%">						
                          <option value="">-All Itemreviewer-</option>
                            <?php							
                            foreach($itemreviewers as $itemreviewer)
                            {
                              $sel = "";								
                              if(isset($item_reviewedby) && $item_reviewedby == $itemreviewer['admin_id']) { $sel = 'selected="selected"'; }
                              echo '<option value="'.$itemreviewer['admin_id'].'"  '.$sel.' >'.$itemreviewer['firstname'].' '.$itemreviewer['lastname'].' ('.$itemreviewer['username'].')</option>';
                            }
                            ?>
                         </select>
                    </div>
...
$param3 = (isset($item_reviewedby)&&$item_reviewedby!="")?$item_reviewedby:0;
...
<script>
  ...
  var table = $('#na_datatable').DataTable( {
    "processing": true,
    "serverSide": true,
    "ajax": "<?=base_url('admin/items/datatable_json_revp_ss_searched/'.$param1.'_'.$param2.'_'.$param3);?>",
    "order": [[0,'desc']],     
    "columnDefs": [
