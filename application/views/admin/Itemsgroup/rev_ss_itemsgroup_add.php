      ...
      <div class="card card-default color-palette-bo">
        <div class="card-header">
          ...
          <div class="d-inline-block float-right">
            <a href="<?= base_url('admin/itemsgroup/rev_ss_pitemsgroup'); ?>" class="btn btn-success"><i class="fa fa-list"></i>  Item Group List</a>
          </div>
        </div>
        <div class="card-body">   
           <!-- For Messages -->
            <?php $this->load->view('admin/includes/_messages.php') ?>
            <?php echo form_open(base_url('admin/itemsgroup/rev_ss_add'), 'class="form-horizontal" enctype="multipart/form-data" onsubmit="return checkItems();" ');  ?>  
...
$('#group_subject_id').on('change', function() {	
	$.post('<?=base_url("admin/itemsgroup/rev_all_items_by_subject")?>',
    {
      '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
 ...
 $('#group_item_1 option:not(:first)').remove();
      ...
      $('#group_item_1')
         .append($("<option></option>")
                    ...
                    .text(value.item_id+' - '+value.item_code)
                  ); 
       });
 $('#group_item_2 option:not(:first)').remove();
	  $.each(arr, function(key, value) {           
