...
<!------------------------------------------------------------------------------------------------------------------------------------------------------------------------>    
  ...
  <div style="width: 471px; border: 3px solid #000; font-size: 20px; font-family: Times New Roman;">
    <?php
					...
					//die('abc');
					/******************************************/
                    $file_name = 'Control_Files_CRQs_P2/'.$paper_erqs[0]->subject_code.'_G'.($paper_erqs[0]->item_grade_id-2).'_P2_ID'.$paper_erqs[0]->subject_id.'.txt';
					$file_name2 = 'Control_Files_CRQs_IDs_P2/'.$paper_erqs[0]->subject_code.'_G'.($paper_erqs[0]->item_grade_id-2).'_P2_ID'.$paper_erqs[0]->subject_id.'.txt';
					$contents = "";
					$contents2 = "";
					$nctr = 1;
					$nctrx = '';
					/*****************************************************/
						if(!empty($paper_erqs))
						{
							...
							{
								//echo $paper_erq->item_total_marks;
								//die('Marks');
								/**************/
                                $nctrx = str_pad($nctr, 2, '0', STR_PAD_LEFT);
                                $nctr++;
								$contents.= "Item".$nctrx."\t+\t".strtoupper($paper_erq->item_total_marks+1)."\t1".$crp_v."\tY\tp".PHP_EOL;
								$contents2.= $paper_erqs[$i]->item_id."\t+\t".strtoupper($paper_erq->item_total_marks+1)."\t1".$crp_v."\tY\tp".PHP_EOL;
								/***************/
								$i++;
