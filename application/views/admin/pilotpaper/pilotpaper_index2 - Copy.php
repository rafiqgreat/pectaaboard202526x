		...
		<table id="na_datatable" class="table table-bordered table-striped" width="100%" style="border:double; color:#000">
          <thead>
			...
			foreach ($records as $row){
				...
				//$sql = "UPDATE ci_pilot_papers_mcqs SET p_m_versions = ".$p2_mcq_versions.", p_m_versions_distr = '".$version_distributions."', p_m_total_mcqs = ".$p2_mcq_items." WHERE p_subject_id = ".$row['item_subject_id']." AND p_phase = 2";
				//echo $sql;
			...
			foreach ($paper1erq as $row){
				...
				else
				{
					if($row['c_group_count']>0) { 
					 ...
					 echo '<div class="dropdown"><button class="dropbtn">CRQs/ERQs Groups ('.$row['c_group_count'].') &darr;</button><div class="dropdown-content"><a href="'.base_url('admin/pilotpaper/group_erqs_view2/'.$row['subject_id']).'" target="_blank">View Web Paper</a></div></div>';	
						
						// echo '<div class="dropdown"><button class="dropbtn">CRQs/ERQs Groups ('.$row['c_group_count'].') &darr;</button><div class="dropdown-content"><a href="'.base_url('admin/pilotpaper/group_erqs_view/'.$row['subject_id']).'" target="_blank">View Web Paper</a><a href="'.base_url('assets/pilot1_papers/P1_G'.($row['c_grade_id']-2).'_'.$row['subject_code'].'_CRQs.pdf').'" target="_blank">Download PDF</a></div></div>';
