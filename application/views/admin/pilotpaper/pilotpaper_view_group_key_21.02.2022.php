...
<div class="content-wrapper">
  ...
  <div style="width: 471px; border: 3px solid #000; font-size: 20px; font-family: Times New Roman;">
    <?php /*
					...
					$file_name = 'Control_Files_Group_CRQs/'.$paper_groups[0]->subject_name_en.'_G'.($paper_groups[0]->group_grade_id-2).'_V'.$crp_v.'_P1_ID'.$paper_groups[0]->group_subject_id.'.txt';
					$file_name2 = 'Control_Files_Group_CRQs_IDs/'.$paper_groups[0]->subject_name_en.'_G'.($paper_groups[0]->group_grade_id-2).'_V'.$crp_v.'_P1_ID'.$paper_groups[0]->group_subject_id.'.txt';
					$contents = "";
					$contents2 = "";
					$nctr = 1;
					$nctrx = '';
						if(!empty($paper_groups))
						{
							...
							foreach($paper_groups as $paper_group)
							{
                                ...
                                $nctr++;
                                $contents.= "Item".$nctrx."\t+\t".strtoupper($paper_group->item_total_marks+1)."\t1".$crp_v."\tY\tp".PHP_EOL;
								$contents2.= $paper_group[$i]->item_id."\t+\t".strtoupper($paper_group->item_total_marks+1)."\t1".$crp_v."\tY\tp".PHP_EOL;
								$i++;
								if($paper_erq->item_type == 'MCQ')
								{
								?>
    <div style="padding:20px; width: 155px; float: left; border: 1px solid #000;">Q. No.<?php print $i;?> : <?php echo $paper_group->item_option_correct;?></div>
    <?php
								}
							}
                    	}
